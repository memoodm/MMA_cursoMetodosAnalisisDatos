...
# profile.to_file('profile_report Regresion.html')
...
# #Guarda el documeto
# dataFrame_3.to_excel('./DFregresion.xls',index=False)

#%%%%%%%% Escalamiento de data. Para el escalamientod e data empleamos dos metods . Centering y standardization

from sklearn.preprocessing import StandardScaler

#iniciacion del Scaler

# ss=StandardScaler

# Xstd_1= ss.fit_transform(dataFrame_3[["Number of Funding Rounds"]].values)

# # Xstd_1.std()

dataFrame_3["Number of Funding Rounds"] = (dataFrame_3['Number of Funding Rounds'] - dataFrame_3['Number of Funding Rounds'].mean())/dataFrame_3['Number of Funding Rounds'].std()


dataFrame_3["Last Funding Amount Currency (in USD)"] = (dataFrame_3["Last Funding Amount Currency (in USD)"] - dataFrame_3["Last Funding Amount Currency (in USD)"].mean())/dataFrame_3['Last Funding Amount Currency (in USD)'].std()

dataFrame_3["Last Equity Funding Amount Currency (in USD)"] = (dataFrame_3["Last Equity Funding Amount Currency (in USD)"]-dataFrame_3["Last Equity Funding Amount Currency (in USD)"].mean())/dataFrame_3['Last Equity Funding Amount Currency (in USD)'].std()

dataFrame_3["Total Equity Funding Amount Currency (in USD)"]=(dataFrame_3["Total Equity Funding Amount Currency (in USD)"]-dataFrame_3["Total Equity Funding Amount Currency (in USD)"].mean())/dataFrame_3['Total Equity Funding Amount Currency (in USD)'].std()
            
dataFrame_3["Total Funding Amount Currency (in USD)"] = (dataFrame_3["Total Funding Amount Currency (in USD)"]-dataFrame_3["Total Funding Amount Currency (in USD)"].mean())/dataFrame_3['Total Funding Amount Currency (in USD)'].std()
...
# dataFrame_3["Number of Funding Rounds"] = (dataFrame_3['Number of Funding Rounds'] - dataFrame_3['Number of Funding Rounds'].min(axis=0))/(dataFrame_3['Number of Funding Rounds'].max(axis=0)- dataFrame_3['Number of Funding Rounds'].min(axis=0))

# dataFrame_3["Last Funding Amount Currency (in USD)"] = (dataFrame_3["Last Funding Amount Currency (in USD)"] - dataFrame_3["Last Funding Amount Currency (in USD)"].min(axis=0))/(dataFrame_3['Last Funding Amount Currency (in USD)'].max(axis=0)- dataFrame_3["Last Funding Amount Currency (in USD)"].min(axis=0))

# dataFrame_3["Last Equity Funding Amount Currency (in USD)"] = (dataFrame_3["Last Equity Funding Amount Currency (in USD)"]-dataFrame_3["Last Equity Funding Amount Currency (in USD)"].min(axis=0))/(dataFrame_3['Last Equity Funding Amount Currency (in USD)'].max(axis=0)- dataFrame_3["Last Equity Funding Amount Currency (in USD)"].min(axis=0))

# dataFrame_3["Total Equity Funding Amount Currency (in USD)"]=(dataFrame_3["Total Equity Funding Amount Currency (in USD)"]-dataFrame_3["Total Equity Funding Amount Currency (in USD)"].min(axis=0))/(dataFrame_3['Total Equity Funding Amount Currency (in USD)'].max(axis=0)-dataFrame_3["Total Equity Funding Amount Currency (in USD)"].min(axis=0))
            
# dataFrame_3["Total Funding Amount Currency (in USD)"] = (dataFrame_3["Total Funding Amount Currency (in USD)"]-dataFrame_3["Total Funding Amount Currency (in USD)"].min(axis=0))/(dataFrame_3['Total Funding Amount Currency (in USD)'].max(axis=0)-dataFrame_3["Total Funding Amount Currency (in USD)"].min(axis=0))
...
plt.yticks(range(len(Xtrain.columns)), Xtrain.columns)
plt.xticks(range(len(Xtrain.columns)), np.arange(modelo_pca.n_components_) + 1)
plt.grid(False)
plt.colorbar();
...
for x, y in zip(np.arange(len(Xtrain.columns)) + 1, modelo_pca.explained_variance_ratio_):
    label = round(y, 2)
    ax.annotate(
        label,
        (x,y),
...
fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(6, 4))
ax.plot(
    np.arange(len(Xtrain.columns)) + 1,
    prop_varianza_acum,
    marker = 'o'
)

for x, y in zip(np.arange(len(Xtrain.columns)) + 1, prop_varianza_acum):
    label = round(y, 2)
    ax.annotate(
        label,
        (x,y),
...
# ==============================================================================
proyecciones = pca_pipe.transform(X=Xtrain)
...
# proyecciones = np.dot(modelo_pca.components_, scale(Xtrain).T)
# proyecciones = pd.DataFrame(proyecciones, index = ['PC1', 'PC2', 'PC3', 'PC4','PC5','PC6','PC7','PC8','PC9','PC10','PC11','PC12','PC13','PC14','PC15',"PC16"])
# proyecciones = proyecciones.transpose().set_index(Xtrain.index)
# proyecciones.head()
...
X_train, X_test, y_train, y_test = train_test_split(proyecciones, ytrain, test_size=0.3, random_state=0)
