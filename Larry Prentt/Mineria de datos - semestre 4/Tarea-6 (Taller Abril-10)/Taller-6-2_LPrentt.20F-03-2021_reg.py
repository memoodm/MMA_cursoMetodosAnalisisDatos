...
# determinacion de outliers, si los hay
dfz.describe(percentiles=[.25, .5, .75, .90, .95, .99]).to_excel("All5.xlsx")

# determinacion de nulos (nan) por columnas
dfz.isnull().sum().to_excel("All6.xlsx")
round(100*(dfz.isnull().sum()/len(dfz.index)), 2)

# remueva las filas donde Last Equity Funding Amount Currency (in USD) tiene nulos
dfzz = dfz[~np.isnan(dfz['Last Equity Funding Amount Currency (in USD)'])]

# determinacion de nulos (nan) por columnas
dfzz.isnull().sum().to_excel("All7.xlsx")
round(100*(dfzz.isnull().sum()/len(dfzz.index)), 2)


dfzz.corr(method='pearson')

# Mapa de Calor
sns.heatmap(dfzz.corr(),annot=True,cmap="RdYlGn") ######## CLAVE
...
Xtrain = dfzz[[dfz.columns[0]]]

for i in range(1,(dfzz.shape[1])):
    if dfzz.columns[i] != 'y':
...
X = X.drop(['Last Equity Funding Type_Series A'], axis=1)
# X = X.drop(['Funding Status_Late Stage Venture'], axis=1)
X = X.drop([X.columns[16]], axis=1)  # La anterior no funciono y se implemento esta

# Keyword de grafica tomada de Nataly Roa
X.plot(kind='box', subplots=True, figsize=(15,6))
...
Xstd = scaler.fit_transform(X[['Last Funding Amount Currency (in USD)',
                               'Last Equity Funding Amount Currency (in USD)',
                               'Total Equity Funding Amount Currency (in USD)',
                               'Total Funding Amount Currency (in USD)',
                               'Number of Funding Rounds',
                               'Estimated Revenue Range_$1M to $10M',
                               'Number of Employees_51-100']].values)

a=X['Last Funding Amount Currency (in USD)'].std()
b=X['Last Equity Funding Amount Currency (in USD)'].std()
c=X['Total Equity Funding Amount Currency (in USD)'].std()
d=X['Total Funding Amount Currency (in USD)'].std()
e=X['Number of Funding Rounds'].std()
f=X['Estimated Revenue Range_$1M to $10M'].std()
g=X['Number of Employees_51-100'].std()


X['Last Funding Amount Currency (in USD)']=X['Last Funding Amount Currency (in USD)']-X['Last Funding Amount Currency (in USD)'].mean()
X['Last Equity Funding Amount Currency (in USD)']=X['Last Equity Funding Amount Currency (in USD)']-X['Last Equity Funding Amount Currency (in USD)'].mean()
X['Total Equity Funding Amount Currency (in USD)']=X['Total Equity Funding Amount Currency (in USD)']-X['Total Equity Funding Amount Currency (in USD)'].mean()
X['Total Funding Amount Currency (in USD)']=X['Total Funding Amount Currency (in USD)']-X['Total Funding Amount Currency (in USD)'].mean()
X['Number of Funding Rounds']=X['Number of Funding Rounds']-X['Number of Funding Rounds'].mean()
X['Estimated Revenue Range_$1M to $10M']=X['Estimated Revenue Range_$1M to $10M']-X['Estimated Revenue Range_$1M to $10M'].mean()
X['Number of Employees_51-100']=X['Number of Employees_51-100']-X['Number of Employees_51-100'].mean()

###########################################################################
# Centering method to variables with high p-values

X['Last Funding Amount Currency (in USD)']=X['Last Funding Amount Currency (in USD)']/a
X['Last Equity Funding Amount Currency (in USD)']=X['Last Equity Funding Amount Currency (in USD)']/b
X['Total Equity Funding Amount Currency (in USD)']=X['Total Equity Funding Amount Currency (in USD)']/c
X['Total Funding Amount Currency (in USD)']=X['Total Funding Amount Currency (in USD)']/d
X['Number of Funding Rounds']=X['Number of Funding Rounds']/e
X['Estimated Revenue Range_$1M to $10M']=X['Estimated Revenue Range_$1M to $10M']/f
X['Number of Employees_51-100']=X['Number of Employees_51-100']/g
...
# X_train[['Number of Articles','Number of Funding Rounds']]=scaler.fit_transform(X_train[['Number of Articles','Number of Funding Rounds']])
###  esta linea de abajo es la clave
...
# X_train[['Number of Articles']]=scaler.fit_transform(X_train[['Number of Articles']])
# X_train[['Number of Founders']]=scaler.fit_transform(X_train[['Number of Founders']])
# X_train[['Number of Funding Rounds']]=scaler.fit_transform(X_train[['Number of Funding Rounds']])

# X_train[['Last Equity Funding Amount Currency (in USD)']]=scaler.fit_transform(X_train[['Last Equity Funding Amount Currency (in USD)']])
# X_train[['Total Equity Funding Amount Currency (in USD)']]=scaler.fit_transform(X_train[['Total Equity Funding Amount Currency (in USD)']])
# X_train[['Total Funding Amount Currency (in USD)']]=scaler.fit_transform(X_train[['Total Funding Amount Currency (in USD)']])
# X_train[['Number of Investors']]=scaler.fit_transform(X_train[['Number of Investors']])

# X_train[['G2 Stack - Total Products Active']]=scaler.fit_transform(X_train[['G2 Stack - Total Products Active']])

# X_train.head()

sns.heatmap(X_train.corr(),annot=True,cmap="RdYlGn") ######## CLAVE
...
X_test[['Last Equity Funding Amount Currency (in USD)','Total Equity Funding Amount Currency (in USD)','Total Funding Amount Currency (in USD)','Last Funding Amount Currency (in USD)']]=scaler.fit_transform(X_test[['Last Equity Funding Amount Currency (in USD)','Total Equity Funding Amount Currency (in USD)','Total Funding Amount Currency (in USD)','Last Funding Amount Currency (in USD)']])
...
X_trainP, X_testP, y_trainP, y_testP = train_test_split(proyecciones, y, test_size=0.3, random_state=0)
...
smtP = SMOTE(random_state=0)  # Muestra de entrenamiento balanceado con variable binaria
data_XP,data_yP=smtP.fit_sample(X_trainP, y_trainP) # en misma proporcion

sns.countplot(x=data_XP['PC1'],data=data_yP)
plt.title('No Exitosas vs Exitosas')
plt.show()
...
log_reg = sm.Logit(data_yP, data_XP).fit(method='newton', maxiter=200)

print(log_reg.summary())

summary = log_reg.summary()

file = open("summaryF-4-4-21_PCA.txt", "w")
str_summary = repr(summary)
file.write("summary = " + str_summary + "\n")
# "\n" creates newline for next write to file
...
yhatP = log_reg.predict(X_testP)
prediction = list(map(round, yhatP))
print('Actual values', list(y_testP.values)) 
print('Predictions :', prediction)

from sklearn.metrics import (confusion_matrix,  
                           accuracy_score)
cm = confusion_matrix(y_testP, prediction)

print ("Confusion Matrix : \n", cm)
print('Test accuracy = ', accuracy_score(y_testP, prediction))
##############################################################

######################## Tested Values
yhatt = log_reg.predict(data_XP)
predictiont = list(map(round, yhatt))
cm = confusion_matrix(data_yP, predictiont)

print ("Confusion Matrix : \n", cm)
print('Test accuracy = ', accuracy_score(data_yP, predictiont))
###############################################################

# dataFrame_3["Number of Funding Rounds"] = dataFrame_3['Number of Funding Rounds'] - dataFrame_3['Number of Funding Rounds'].mean()
