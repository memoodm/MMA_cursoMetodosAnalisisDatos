...
# Nuevas graficas del punto #2, despues de clase del 10 de Abril

pd.crosstab(dfx2.Pais,dfx2['Number of Employees']).plot(kind='bar')
plt.title('Number of Employees - Total  per Country in the last 6 years', fontsize=22)
plt.xlabel('Country', fontsize=20)
plt.ylabel('Number of Employees counting', fontsize=20)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
plt.show()


pd.crosstab(dfx2.Pais,dfx2['Number of Employees'],aggfunc="sum",values=dfx2['Last Funding Amount Currency (in USD)']).plot(kind='bar')
plt.title('Last Funding Amount per number of employees Range - Total  per Country in the last 6 years', fontsize=22)
plt.xlabel('Country', fontsize=20)
plt.ylabel('Last Funding Amount in MM - USD', fontsize=20)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
...
pd.crosstab(dfx2.Pais,dfx2['Number of Investors']).plot(kind='bar')
plt.title('Number of Investors - Total  per Country in the last 6 years', fontsize=22)
plt.xlabel('Country', fontsize=20)
plt.ylabel('Number of Investors counting', fontsize=20)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
plt.show()

pd.crosstab(dfx2.Pais,dfx2['Number of Investors'],aggfunc="sum",values=dfx2['Last Funding Amount Currency (in USD)']).plot(kind='bar')
plt.title('Last Funding Amount per Number of Investors Range - Total  per Country in the last 6 years', fontsize=22)
plt.xlabel('Country', fontsize=20)
plt.ylabel('Last Funding Amount in MM - USD per number of investors category', fontsize=20)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
plt.show()


pd.crosstab(dfx2.Pais,dfx2['Number of Funding Rounds']).plot(kind='bar')
plt.title('Number of Funding Rounds - Total  per Country in the last 6 years', fontsize=22)
plt.xlabel('Country', fontsize=20)
plt.ylabel('Number of Funding Rounds counting', fontsize=20)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
plt.show()


pd.crosstab(dfx2.Pais,dfx2['Number of Funding Rounds'],aggfunc="sum",values=dfx2['Last Funding Amount Currency (in USD)']).plot(kind='bar')
plt.title('Last Funding Amount per Number of Funding Rounds Range - Total  per Country in the last 6 years', fontsize=22)
plt.xlabel('Country', fontsize=20)
plt.ylabel('Last Funding Amount in MM - USD per Number of Funding Rounds category', fontsize=20)
plt.xticks(fontsize=18)
plt.yticks(fontsize=18)
plt.show()
...
splot =sns.scatterplot(data=data, x='TEFA (USD)', y='Number of Exits', hue='Pais')
splot.set(xscale="log")
splot.set(yscale="log")
...
# interseccion de 2 dataframes a traves de columnas Organization y Company. CENTER
merged_inner = pd.merge(left=dfx, right=df12, left_on='Organization', right_on='Company')


splot =sns.scatterplot(data=merged_inner, x='Last Funding Amount Currency (in USD)', y='Valuation ($B) ', hue='Pais')
splot.set(xscale="log")
splot.set(yscale="log")


# interseccion de 2 dataframes a traves de columnas Organization y Company. LEFT
merged_left = pd.merge(left=dfx, right=df12, how = 'left', left_on='Organization', right_on='Company')
...
df4A=dfy["Industry Groups"].str.split(",", n = 11, expand = True)

for i in range(df4A.shape[1]):
    df4A[i] = df4A[i].astype(str)
    df4A[i] = df4A[i].str.strip()
...
list8 = df4A[7].tolist()
list9 = df4A[8].tolist()
list10 = df4A[9].tolist()
list11 = df4A[10].tolist()
list12 = df4A[11].tolist()

# inter=set(list1)&set(list2)&set(list3)&set(list4)&set(list5)&set(list6)
inter=set(list1)&set(list2)&set(list3)

#####################################################  Especial 1

dfy['Industries2'] = df4A[0]
dfy['Industries2'] = dfy['Industries2'].replace('nan', 'Other')
dfy['Industries2'].unique()

tabla2 = pd.pivot_table(dfy, 'Last Funding Amount Currency (in USD)','Pais','Industries2', aggfunc=np.sum )/1e6
...
plt.title('Last Funding Amount - Industries - Pais', fontsize = 22)
...
tabla1 = pd.pivot_table(dfy, values=["Last Equity Funding Amount Currency (in USD)"], index=["Industries2"],  aggfunc=np.sum)/1e6
tabla1.plot(kind="bar")
plt.xlabel('Sectores', fontsize=18)
...
tabla5 = pd.pivot_table(dfy, values=["Last Funding Amount Currency (in USD)"], index=["Pais","Headquarters Location","year"],aggfunc=np.sum)/1e6
tabla6 = pd.pivot_table(dfy, values=["Organization"], index=["Pais","Headquarters Location", "year"],aggfunc="count")
...
tabla_merge.rename(columns={"Organization": 'deals'}, inplace=True)

tabla_merge=tabla_merge[tabla_merge.columns[:]][(tabla_merge["year"] >= 2018) & (tabla_merge["year"] <= 2020)]


pd.crosstab(tabla_merge.Pais,tabla_merge['year'],aggfunc="sum",values=tabla_merge['deals']).plot(kind='bar')
plt.title('# of Deals per Country in the last 3 years', fontsize=22)
plt.xlabel('Country', fontsize=20)
...
sns.displot(
    tabla_merge, x="deals", col="Pais", row="year",
    binwidth=3, height=3, facet_kws=dict(margin_titles=True),
)

# No Sale
# sns.displot(
#     tabla_merge, x="LFA", col="Pais", row="year",
#     binwidth=3, height=3, facet_kws=dict(margin_titles=True),
# )


#####################################################  Grafica Especial Pedida en Clase
def plot_especial(dataf,pais):
    print(pais)
    print(str(pais))
    tabla51 = pd.pivot_table(dataf, values=["Last Funding Amount Currency (in USD)"], index=["Pais","year"],aggfunc=np.sum)/1e6
    tabla61 = pd.pivot_table(dataf, values=["Headquarters Location"], index=["Pais","year"],aggfunc="count")
    
    tabla_merged= pd.concat([tabla51, tabla61], 1).dropna().mean(axis=1, level=0)
    tabla_merged = tabla_merged.reset_index()
    
    tabla_merged.rename(columns={"Last Funding Amount Currency (in USD)": 'LFA'}, inplace=True)
    
    tabla_merged=tabla_merged[tabla_merged.columns[:]][tabla_merged["Pais"] == str(pais)]
    
    fig, ax = plt.subplots()
    ax2 = ax.twinx()
    ax.bar(tabla_merged.year, tabla_merged.LFA, color=(190/255,190/255,190/255,0.7), label='Last Funding Amount Currency (in USD)')
    ax2.plot(tabla_merged.year, tabla_merged["Headquarters Location"], color='green', label='Headquarters Location')
    ax.set_xticklabels(tabla_merged.year, fontsize=18)
    ...
    plt.title(pais, fontsize=22)
    return
##############################################################    

plot_especial(dfy,'israel')
...
dict_col_zeros=cols_90per_zeros(dfz, 0.95)
